 ...
 *
 */
public class XNDiscMovProcessor extends XNDiscBaseProcessor {

	public XNDiscMovProcessor(StorageIF storage, LoggerIF logger, StringTokenizer files, int filecounts) {
	...
	public boolean run(INonBlockingPipeline connection) throws BufferUnderflowException, BufferOverflowException, ClosedChannelException, IOException {
		...
		boolean rtn = false;
		String msg = "";
		logger.log(LoggerIF.LOG_INFO, "*** [" + connection.getId() + "] [" + connection.getRemoteAddress().getHostAddress() + ":" + connection.getRemotePort() + "] file move start !!!!");
		...
		try {
			...
			nFile = XNDiscUtils.retrieveMovNFile(nFile, arrOriFilter);
		...
		logger.log(LoggerIF.LOG_INFO, "*** [" + connection.getId() + "] [" + connection.getRemoteAddress().getHostAddress() + ":" + connection.getRemotePort() + "] file move end !!!!");
		return rtn;
	}
