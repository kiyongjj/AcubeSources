...
<%
	...
	String rqv_flag   = request.getParameter("rqv_flag") == null ? "false" : request.getParameter("rqv_flag");
...
<script language="JavaScript" for="RQViewer" event="EndRunReport">
	...
	<%
		}else if(rqv_flag.equals("save")){
	%>
			RQViewer.SaveAs("rqv", "<%=saveDir%>" + "/" + lm_docname + ".rqv", 0, 0,""); // save rqv file
	<%
		}
	%>
	}
	//alert("EndRunReport");
	<%
		// Print Excel Cancel 은 이벤트 처리를 하여 cnt++ 을 해줌
		// pdf, rqv 는 따로 해주지 않고 여기서
		if(pdf_flag.equals("show") || pdf_flag.equals("save") || rqv_flag.equals("save")){
	%>
...
{	
	...
	<%	
		if (print_flag.equals("true")) {
	%>
			oRQPlugin.Print(false, 0, 0, 1);
	<%
		}else if(excel_flag.equals("true") || excel_flag.equals("show")){
	%>
			oRQPlugin.SendToExcel("", true, 0, true); // show excel
	<%
		}else if(excel_flag.equals("save")){
	%>
			oRQPlugin.SendToExcel("<%=saveDir%>" + "/" + lm_docname + ".xls"  , true, 0, false); // save excel file
	<%
		}else if(pdf_flag.equals("show")){
	%>
		  	oRQPlugin.SendToPdf("", true); // show pdf
	<%
		}else if(pdf_flag.equals("save")){
	%>	
			oRQPlugin.SendToPdf("<%=saveDir%>" + "/" + lm_docname + ".pdf", false); // save pdf file
	<%
		}else if(rqv_flag.equals("save")){
	%>
			oRQPlugin.SaveAs("rqv", "<%=saveDir%>" + "/" + lm_docname + ".rqv", 0, 0,""); // save rqv file
	<%
		}
	%>
	}
