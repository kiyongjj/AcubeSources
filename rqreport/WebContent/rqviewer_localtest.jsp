...
function OnRun() {
	...
	<% if(viewer_type.equals("ocx")){ %>
		...
		else
		{
			try
			{
				...
				oReport = RQViewer.OpenReport("<%=basePath%>rqcheck/rqvdownload/RQdownload_localtest.jsp?filename=" + docName);
		...
		if (!oReport.IsResultFile())
		{
			...
			OConnObj.SetPath("<%=basePath%>RQDataset_localtest.jsp");
		}
		else	// 결과 파일일 경우
		{
	...
	<% }else{ %>
		...
		//---SQLExec -------------------------------------------------
		oRQPlugin.SetSQLExecURL("<%=basePath%>RQDataset_ghr.jsp",docName);
