...
function OnRun() {
	...
	<% if(viewer_type.equals("ocx")){ %>
		...
		else
		{
			...
			{	
			    ...
			    oReport = RQViewer.OpenReport("<%=basePath%>document/getreport_chkid_getresource.jsp?doc=" + docName);
		...
		if (!oReport.IsResultFile())
		{
			...
			OConnObj.SetPath("<%=basePath%>RQDataset_chkid_getresource.jsp");
		}
		else	// 결과 파일일 경우
		{
	...
	<% }else{ %>
		...
		//---SQLExec -------------------------------------------------
		oRQPlugin.SetSQLExecURL("<%=basePath%>RQDataset_chkid_getresource.jsp",docName);
