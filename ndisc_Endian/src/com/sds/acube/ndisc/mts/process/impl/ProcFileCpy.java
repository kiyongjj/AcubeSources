...
public class ProcFileCpy extends DefProcessAdaptor {
   public ProcFileCpy(LoggerIF logger, StorageIF storage) {
      super(logger, storage);
   }

   public boolean process(Object obj1, Object obj2) {
	   logger.log(LoggerIF.LOG_DEBUG, "ProcFileCpy.process() started !!!!");
      boolean bRet = false;
      String strMsg = null;
      ...
      try {
         ...
         if (true == bRet) {
            ...
            for (int i = 0; i < nFile.length; i++) {
               strMsg = strMsg + nFile[i].getId() + NDCommon.DELIM_STR;
            }
         } else {
      ...
      logger.log(LoggerIF.LOG_DEBUG, "ProcFileCpy.process() ended !!!!");
      return true;
   }
